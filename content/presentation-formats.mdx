...
Open `github-presentation-template.key` or `github-presentation-theme.kth` to create a new presentation.

**Note:** If you just open the regular keynote file (`.key`), you will need to duplicate this for your own presentation if you do not want to override the template for later use. We recommend that for your first presentation you open the theme file (`.kth`) and add the theme to your Keynote to use for future use.
...
You can create a new presentation by using either the `.potx` file or the `.pptx` file.
...
**Tip:** We recommend using the template(`.potx`) file as this will start a fresh document with everything loaded right away. 😄
...
The GitHub presentation template is available for use in the Google Slides template gallery for members of the GitHub organization.
